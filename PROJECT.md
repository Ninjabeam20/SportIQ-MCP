...
- **`src/sportiq/core/`** — everything sport-agnostic:
  ...
  - `ratelimit.py` — `Budget(source, per_minute, per_day)`, `reserve()` (atomic admit before
    fetch), `refund()` (failed fetch), `has_budget()` (peek/diagnostics), `consume()`
    (unconditional incr for tests), `remaining()` (health). Budget math assumes one process.
...
871 collected tests, four layers (see `.claude/rules/testing.md`):
...
- `src/sportiq/football/data/*.json` — regenerated by `scripts/build_*.py`, never hand-edited.
  ...
  bracket simulation.
- `pyproject.toml` — the sdist **allowlist** + `check_release_build.py` are the only
...
5. **Version lives in three places:** `pyproject.toml` (source of truth; `__init__.__version__`
   reads package metadata), `server.json` (MCP registry manifest), and FastMCP
