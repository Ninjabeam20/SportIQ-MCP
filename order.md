...
- [x] 2a. Football: `adapters/api_football.py:90-107` (standings), `:177-195` (scorers), `adapters/football_data_org.py:30-132` (fixtures/standings/scorers), `adapters/openfootball.py:28-48`, `adapters/static_seed.py:42-50` — raise `NotFoundError` on empty payloads (mirror fixtures/squad guards). Fix `football_data_org.py:93-107` TeamStats (wrong endpoint / ID space — raise or drop from chain). **PARTIAL** — api_football standings/scorers empty + football_data_org empty DONE (`77bef8f`/`b395f15`); leave openfootball/static_seed/TeamStats unchecked if not done (2026-09-11).
...
- [ ] 2c. `football/adapters/derived_standings.py:26-31` — route via fixtures chain/cache instead of direct adapter `fetch()`.
...
| Status | File | Change |
| :--- | :--- | :--- |
| [x] | `src/sportiq/football/adapters/api_football.py` | 2a: standings/scorers empty `NotFoundError` — **DONE** `77bef8f`/`b395f15` (TeamStats may remain) |
| [x] | `src/sportiq/football/adapters/football_data_org.py` | 2a: empty raises — **PARTIAL DONE** `77bef8f`/`b395f15`; TeamStats/4c may remain |
| [ ] | `src/sportiq/football/adapters/openfootball.py` | 2a: raise on empty `matches` | — leave open (2026-09-11)
| [ ] | `src/sportiq/football/adapters/static_seed.py` | 2a: raise on missing file (groups/fixtures loaders only) | — leave open (2026-09-11)
| [ ] | `src/sportiq/football/adapters/derived_standings.py` | 2c: route via `football_fixtures_chain` |
| [ ] | `src/sportiq/football/chains.py` | 2a/4c: drop-or-keep FD team-stats decision; `str(team)` key; register derived for health |
