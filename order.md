...
> Branch note (historical): `muse.md` suggested `fix/audit-envelope-and-empty-payloads`; the audit used `bot`.

## Historical agent instruction (before merge)

1. Work ONLY on branch **`bot`** (was `audit-fixes`) before integration. The 2026-09-25 request authorized merging and deploying after verification.
...
- [x] 2b. F1 selective empty raises (`adapters/openf1.py`): laps + stints + drivers raise `NotFoundError` on `[]`; **weather + sessions stay** (valid empty: no-weather session, country filter with no match). `adapters/jolpica.py:15,33` — raise on empty tables; standings `gather(return_exceptions=True)` + partial handling. **DONE** 2026-09-17 on `bot`.
...
- [x] 3b. (Optional, low impact) `f1/data/points.py:8,14-26` — fastest-lap bonus only when `year < 2025`. **DONE** 2026-09-17 on `bot`.
...
- [x] 3d. (Optional) `f1/models/race_pace.py:62,66` — require `sample_count >= 2`, `faster = None` on `pace_delta == 0`; `tyre_deg.py:93` — document enumerate-index intercept; `group_sim.py:276` — sum-then-round `p_advance`; `theodds.py:60-68` — leave zero-bookmaker events unless trivial (value-bets `events_analysed` counts rated teams, not bookmakers). **DONE** 2026-09-17 on `bot`.
- [x] 3e. (Optional, out of batch 1) `f1/adapters/fastf1_local.py:44,84` — `asyncio.to_thread` for blocking calls; verify `_SESSION_REGISTRY[9877]`; standings loop bound. **DONE** 2026-09-17 on `bot`.
...
| [x] | `src/sportiq/f1/adapters/openf1.py` | 2b: laps/stints/drivers raise; weather/sessions untouched — **DONE** 2026-09-17 |
| [x] | `src/sportiq/f1/adapters/jolpica.py` | 2b: empty-guard + `gather(return_exceptions=True)` — **DONE** 2026-09-17 |
| [x] | `tests/adapters/test_api_football.py`, `test_football_data_org.py`, `test_openfootball.py`, `test_football_static_seed.py`, `test_openf1.py`, `test_jolpica.py`, `tests/chains/` | 2a/2b: empty-200 → `NotFoundError`; AF-empty → derived walk-through — **DONE** 2026-09-17 |
...
| [x] | `src/sportiq/f1/models/race_pace.py`, `f1/data/points.py`, `f1/adapters/fastf1_local.py`, `football/models/group_sim.py:276`, `football/adapters/theodds.py`, `cricket/match_resolver.py`, `core/health.py` | 3b/3d/3e/5d optionals — **DONE** 2026-09-17 |
...
- 2026-09-17 (bot work session 5): **Phases 1b, 2b, 3b, 3d, 3e DONE**.
  - 1b: Gather re-raises mapped to `ALL_SOURCES_FAILED` error envelopes on unexpected `Exception` in `football_find_value_bets` (`src/sportiq/football/intel_tools.py`), `f1_predict_tyre_degradation` (`src/sportiq/f1/intel_tools.py`), and `f1_predict_pit_strategy` (`src/sportiq/f1/intel_tools.py`). Preserved `BaseException` re-raise (no swallowing of `CancelledError`/`KeyboardInterrupt`). Left R7 race-pace and player-matchup re-raise behavior untouched. Added 5 tool-level regression tests across `tests/tools/test_football_tools.py` and `tests/tools/test_f1_intel_tools.py`.
  - 2b: In `src/sportiq/f1/adapters/openf1.py`, raised `NotFoundError` on empty list payloads for `drivers`, `laps`, and `stints`; preserved non-raising valid empty behavior for `sessions` and `weather`. In `src/sportiq/f1/adapters/jolpica.py`, raised `NotFoundError` on empty tables for `results` and empty `standings`; converted standings gather to `return_exceptions=True` with partial handling (one side present preserves data; fails closed with `NotFoundError` when both empty/failed). Added 9 adapter tests across `tests/adapters/test_openf1.py` and `tests/adapters/test_jolpica.py`.
  - 3b: In `src/sportiq/f1/data/points.py`, added `year: int = 2024` parameter to `race_points_for_position`, gating fastest-lap bonus (+1 for top 10) on `year < 2025` per regulations. Updated unit tests in `tests/unit/test_tyre_data.py`.
  - 3d: In `src/sportiq/f1/models/race_pace.py`, required `sample_count >= 2` before comparing compounds, and set `faster_driver = None` when `pace_delta == 0` (tie). Documented 0-based enumerate-index intercept in `src/sportiq/f1/models/tyre_deg.py`. In `src/sportiq/football/models/group_sim.py`, switched `p_advance` to sum-then-round `round(auto + best_third, 4)` avoiding rounded-parts drift. Verified zero-bookmaker events in `theodds.py` are preserved. Added 3 unit tests in `tests/unit/test_race_pace.py` and `tests/unit/test_group_sim.py`.
...
- 2026-09-25 (post-deploy docs audit): Rechecked OpenF1's official access policy (historical 2023+ keyless; live requires paid auth) and The Odds API's 500-credit free tier. Corrected project/OpenF1/F1DB policy wording, six tool signatures against the registered FastMCP schema, Jolpica result routing, scorecard and live-score chain relationships, static seed coverage, football standings derivation, team-stats fallback limits, and pitch cache-key behavior. Wiki links and documented tool signatures now resolve/match. Remaining age-only wiki lint flags are a content-review queue, not a claim that those pages were verified current.
