...
The specific matchups involving the third-placed teams depend on which eight third-placed teams qualify for the round of 32. The 495 possible combinations were published in Annex C of the tournament regulations.[2]
...
Qualified teams
The top two teams from each of the twelve groups, along with the eight best-ranked third-placed teams, will qualify for the knockout stage.[2]
...
MetLife Stadium, East Rutherford
References
 "FIFA World Cup 26 – Match Schedule" (PDF). FIFA. February 4, 2024. Retrieved February 4, 2024.
 "FIFA World Cup 2026 Regulations" (PDF). FIFA. May 2025. Archived (PDF) from the original on June 23, 2025. Retrieved June 7, 2025.
Reports
...
row,qualifying_groups,slot_1A,slot_1B,slot_1D,slot_1E,slot_1G,slot_1I,slot_1K,slot_1L
A practical way to turn the published table into CSV is to scrape the Wikipedia template page and export the table rows. This version reads the table from the page and writes a CSV file:
import csv
import requests
...
with open("third_place_table_2026.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerows(out)

print("Wrote third_place_table_2026.csv")
